...
void MemoryManager::printMemoryStatus()
{
    ...
    int kpTotal = kernelPhysical.resources.size();
    int kpUsed = kernelPhysical.resources.getUsedCount();
    int kpMaxFree = kernelPhysical.resources.getMaxFreeBlock();
    int kpFrags = kernelPhysical.resources.getFreeFragmentCount();

    printf("kernel physical pool:\n"
           "    used/total: %d/%d pages (%d%%)\n"
           "    max free block: %d pages\n"
           "    free fragments: %d\n",
           kpUsed, kpTotal,
           kpTotal ? (kpUsed * 100 / kpTotal) : 0,
           kpMaxFree, kpFrags);

    int upTotal = userPhysical.resources.size();
    int upUsed = userPhysical.resources.getUsedCount();
    int upMaxFree = userPhysical.resources.getMaxFreeBlock();
    int upFrags = userPhysical.resources.getFreeFragmentCount();

    printf("user physical pool:\n"
           "    used/total: %d/%d pages (%d%%)\n"
           "    max free block: %d pages\n"
           "    free fragments: %d\n",
           upUsed, upTotal,
           upTotal ? (upUsed * 100 / upTotal) : 0,
           upMaxFree, upFrags);

    int kvTotal = kernelVirtual.resources.size();
    int kvUsed = kernelVirtual.resources.getUsedCount();
    int kvMaxFree = kernelVirtual.resources.getMaxFreeBlock();
    int kvFrags = kernelVirtual.resources.getFreeFragmentCount();
